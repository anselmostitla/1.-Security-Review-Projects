...
contract Handler is Test {
   ...
   function swapPoolTokenForWethBaseOnOutputWeth(uint256 outputWeth) public {
      ...
      if(poolTokenAmount > poolToken.balanceOf(address(pool))) { return;}
   ...
   function deposit(uint256 wethAmount) public {
      ...
      pool.deposit(wethAmount, pool.getMinimumWethDepositAmount(), uint256(expectedDeltaX), uint64(block.timestamp));
      vm.stopPrank();
